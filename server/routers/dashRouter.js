...
const removeFile = multer({ dest: './../uploads/' });
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (validUser) {
      ...
      db.logSuccess("Access accepted");
    } else {
      db.logError("Pass is wrong");
...
router.get("/", (req, res) => {
  ...
  if (isPersistentLoggedIn || isSessionLoggedIn) {
    db.logSuccess("User is authenticated");
...
router.post("/add/user", upload.single("image"), async (req, res) => {
  ...
  if (isPersistentLoggedIn || isSessionLoggedIn) {
    ...
    try {
      ...
      await db.saveData("Users", newData);
...
// Delete data by ID
router.delete("/:id", removeFile.single("image"),async (req, res, next) => {
  const id = req.params.id;
...
router.post("/edit/:id", upload.single("image"), async (req, res) => {
  ...
  if (isPersistentLoggedIn || isSessionLoggedIn) {
    ...
    try {
      ...
      await db.editByID(id, dataName, updatedData);
      res.redirect(`/dash`);
    } catch (err) {
      db.logError(err);
      res.status(500).render("500-2.ejs", {
...
router.post("/edit/user/:userId", upload.single("image"), async (req, res) => {
  ...
  if (isPersistentLoggedIn || isSessionLoggedIn) {
    ...
    try {
      ...
      const dataName = "Users";
      await db.editById(userId, dataName, updatedData);
      res.redirect(`/dash`);
    } catch (err) {
      db.logError(err);
      res.status(500).render("500-2.ejs", {
